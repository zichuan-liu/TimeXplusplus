...
def main(args):
    ...
    if Dname == 'epilepsy':
        trainEpi, val, test = process_Epilepsy(split_no = args.split_no, device = device, base_path = '/TimeX/datasets/Epilepsy/')
        test = (test.X, test.time, test.y)
        trainX = trainEpi.X
    elif Dname == 'pam':
        trainPAM, val, test = process_PAM(split_no = args.split_no, device = device, base_path = '/TimeX/datasets/PAM/', gethalf = True)
        test = (test.X, test.time, test.y)
        trainX = trainPAM.X
    elif Dname == 'boiler': # TODO: fill
        trainB, val, test = process_Boiler_OLD(split_no = args.split_no, device = device, base_path = '/TimeX/datasets/Boiler/')
        trainX = trainB[0]
    else:
    ...
    if args.exp_path is not None:
        ...
        if args.exp_method == 'ours' or args.exp_method == "timex":
...
# python occlusion_exp.py --dataset pam --exp_method dyna
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--exp_method', type = str, help = "Options: ['random', 'ig', 'dyna', 'winit', 'ours']", default='ours')
    ...
    for xxx in range(4, 6):
        ...
        if args.exp_method=='ours' or args.exp_method=='timex':
            if args.dataset=="pam":
                args.model_path = "/TimeX/experiments/PAM/models/{}_full_split={}.pt".format(add_str,fold)
            elif args.dataset=="epilepsy":
                args.model_path = "/TimeX/experiments/epilepsy/models/{}_full_split={}.pt".format(add_str,fold)
            elif args.dataset=="boiler":
                args.model_path = "/TimeX/experiments/Boiler/models/{}_full_split={}.pt".format(add_str,fold)
        else:
            if args.dataset=="pam":
                args.model_path = "/TimeX/experiments/PAM/models/transformer_split={}.pt".format(fold)
            elif args.dataset=="epilepsy":
                args.model_path = "/TimeX/experiments/epilepsy/models/transformer_split={}.pt".format(fold)
            elif args.dataset=="boiler":
                args.model_path = "/TimeX/experiments/Boiler/models/transformer_split={}.pt".format(fold)
        ...
        if args.exp_method == 'random':
            ...
            savepath = '{}_{}_sp={:d}_occlusion_results.csv'.format(args.dataset, args.exp_method, xxx)
